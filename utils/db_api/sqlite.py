...
def add_userx(user_id, user_login, user_name, balance, all_refill, reg_date, ref_code):
    with sqlite3.connect(path_to_db) as db:
        db.execute("INSERT INTO storage_users "
                   "(user_id, user_login, user_name, balance, all_refill, reg_date, ref_code) "
                   "VALUES (?, ?, ?, ?, ?, ?, ?)",
                   [user_id, user_login, user_name, balance, all_refill, reg_date, ref_code])
        db.commit()
...
def get_all_usersx():
    with sqlite3.connect(path_to_db) as db:
        get_response = db.execute("SELECT * FROM storage_users ORDER BY all_pay")
        get_response = get_response.fetchall()
    return get_response
...
def get_settingsp(p):
    with sqlite3.connect(path_to_db) as db:
        get_response = db.execute(f"SELECT {p} FROM storage_settings")
        get_response = get_response.fetchone()[0]
    return get_response
...
def get_all_podcategories1(up_cat):
    with sqlite3.connect(path_to_db) as db:
        sql = "SELECT * FROM storage_category"
        result = db.execute("SELECT * FROM storage_category WHERE upcategories = ?", (up_cat,))
        get_response = get_response.fetchall()
    return get_response

def get_all_categoriesx(**kwargs):
    with sqlite3.connect(path_to_db) as db:
        sql = "SELECT * FROM storage_category WHERE "
        sql, parameters = get_format_args(sql, kwargs)
        get_response = db.execute(sql, parameters)
        get_response = get_response.fetchall()
    return get_response

def get_all_categoriesx_creat():
    with sqlite3.connect(path_to_db) as db:
        sql = "SELECT * FROM storage_category"
        get_response = db.execute(sql)
        get_response = get_response.fetchall()
...
def add_positionx(position_id, position_name, position_price, position_discription, position_image, position_date,
    ...
    with sqlite3.connect(path_to_db) as db:
        db.execute("INSERT INTO storage_position "
                   "(position_id, position_name, position_price, position_discription, position_image, position_date, category_id, tovar) "
                   "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                   [position_id, position_name, position_price, position_discription, position_image,
                    position_date, category_id, tovar])
        db.commit()
...
# Добавление категории в БД
def add_itemx(summ, name, kol, val):
    with sqlite3.connect(path_to_db) as db:
        db.execute("INSERT INTO storage_item "
        "(item_id, item_data, kol, kol_get, val) "
        "VALUES (?, ?, ?, ?, ?)",
        [summ, name, kol, 0, val])
        db.commit()
...
def update_itemx(item_id, **kwargs):
    with sqlite3.connect(path_to_db) as db:
        sql = f"UPDATE storage_item SET XXX WHERE increment = {item_id}"
        sql, parameters = update_format_with_args(sql, kwargs)
        db.execute(sql, parameters)
        db.commit()
...
def add_purchasex(user_id, user_login, user_name, receipt, item_count, item_price, item_price_one_item,
                  item_position_id,
                  item_position_name, item_buy, balance_before, balance_after, buy_date, buy_date_unix, url):
    with sqlite3.connect(path_to_db) as db:
    ...
    if True:
        with sqlite3.connect(path_to_db) as db:
            ...
            if len(check_create_users) == 8:
                ...
                db.execute("CREATE TABLE IF NOT EXISTS storage_users("
            ...
            if len(check_create_payment) == 6:
                ...
                db.execute("CREATE TABLE IF NOT EXISTS storage_payment("
            ...
            if len(check_create_settings) == 11:
                print("DB was found(3/8)")
            else:
                db.execute("CREATE TABLE IF NOT EXISTS storage_settings("
                           "contact INTEGER, faq TEXT, "
                           "status TEXT, status_buy TEXT,"
                           "profit_buy TEXT, profit_refill TEXT, sber_rek STRING, sber_min INTEGER, tovar STRING, com INTEGER, curs REAL)")
                sql = "INSERT INTO storage_settings(" \
                      "contact, faq, status, status_buy, profit_buy, profit_refill, sber_rek, sber_min, tovar, com, curs) " \
                      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                ...
                parameters = ("ℹ Контакты. Измените их в настройках бота.\n"
                              ...
                              "➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                              f"{bot_description}",
                              "True", "True", now_unix, now_unix, "", 10, "", 25, 0.66)
                db.execute(sql, parameters)
            ...
            if len(check_create_refill) == 10:
                ...
                db.execute("CREATE TABLE IF NOT EXISTS storage_refill("
                           "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
            ...
            if len(check_create_category) == 3:
                ...
                db.execute("CREATE TABLE IF NOT EXISTS storage_category("
                           "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                           "category_id INTEGER, category_name TEXT, upcategories INTEGER)")
            ...
            if len(check_create_position) == 8:
                ...
                db.execute("CREATE TABLE IF NOT EXISTS storage_position("
                           "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
            ...
            if len(check_create_item) == 6:
                print("DB was found(7/8)")
            else:
                db.execute("CREATE TABLE IF NOT EXISTS storage_item("
                           "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
            ...
            if len(check_create_purchases) == 15:
                ...
                db.execute("CREATE TABLE IF NOT EXISTS storage_purchases("
                           "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                           "user_id INTEGER, user_login TEXT, "
