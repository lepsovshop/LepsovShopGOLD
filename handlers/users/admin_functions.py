...
from utils.db_api.sqlite import get_purchasex, get_refillx, update_userx, last_purchasesx, get_all_usersx, add_refillx
...
async def send_ad_all_users(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("📢 <b>Введите текст для рассылки пользователям:</b>", parse_mode='HTML')
...
async def input_text_for_ad(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data["here_send_ad"] = str(message.text)
    ...
    await bot.send_message(message.from_user.id,
                           ...
                           reply_markup=sure_send_ad_inl, parse_mode='HTML')
...
async def sends_ad(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "not_send_kb":
        await state.finish()
        await call.message.answer("<b>📢 Вы отменили отправку рассылки ☑</b>", parse_mode='HTML')
    else:
        await call.message.answer(f"<b>📢 Рассылка началась...</b>, parse_mode='HTML'")
        async with state.proxy() as data:
...
async def input_data_for_search_profile(message: types.Message, state: FSMContext):
    ...
    if get_user_id is not None:
        await message.answer(search_user_profile(get_user_id[1]), reply_markup=search_profile_func(get_user_id[1]), parse_mode='HTML')
        await state.finish()
    else:
    ...
    if len(last_purchases) >= 1:
        ...
        for purchases in last_purchases:
            save_purchases.append(f"<b>📃 Чек:</b> <code>#{purchases[4]}</code>\n"
                                  ...
                                  f"🕜 {purchases[13]}\n"
                                  f"<code>{purchases[10]}</code>", parse_mode='HTML')
...
async def add_balance_user(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer("<b>💴 Введите сумму для выдачи баланса</b>", parse_mode='HTML')
...
async def input_add_balance(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if get_amount >= 1:
            ...
            await bot.send_message(user_id, f"<b>💳 Вам было выдано</b> <code>{get_amount}руб</code>", parse_mode='HTML')
            await message.answer(search_user_profile(user_id), reply_markup=search_profile_func(user_id), parse_mode='HTML')
            await state.finish()
        else:
            await message.answer("<b>❌ Минимальная сумма выдачи 1руб</b>\n"
                                 "💴 Введите сумму для выдачи баланса", parse_mode='HTML')
            await StorageFunctions.here_add_balance.set()
    else:
...
async def set_balance_user(call: CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        data["here_cache_user_id"] = call.data.split(":")[1]
    await call.message.delete()
    await call.message.answer("<b>💸 Введите сумму для изменения баланса</b>", parse_mode='HTML')
    ...
    if message.text.isdigit():
        ...
        if get_amount >= 0:
            ...
            await message.answer("<b>❌ Минимальная сумма баланса 0руб</b>\n"
                                 "💸 Введите сумму для изменения баланса", parse_mode='HTML')
            await StorageFunctions.here_set_balance.set()
    else:
        await message.answer("<b>❌ Данные были введены неверно.</b>\n"
                             "💸 Введите сумму для изменения баланса", parse_mode='HTML')
...
async def send_user_message(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("<b>💌 Введите сообщение для отправки</b>\n"
                              "⚠ Сообщение будет сразу отправлено пользователю.", parse_mode='HTML')
...
async def input_send_user_message(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        user_id = data["here_cache_user_id"]
    get_user = get_userx(user_id=user_id)
    await bot.send_message(user_id, get_message, parse_mode='HTML')
    await message.answer("<b>✅ Пользователю</b> "
                         f"<a href='tg://user?id={get_user[1]}'>{get_user[3]}</a> "
    ...
    if message.text.startswith("+"):
        ...
        if get_input is not None:
            ...
            await message.answer(f"<b>📃 Чек:</b> <code>+{get_input[6]}</code>\n"
                                 ...
                                 reply_markup=get_functions_func(message.from_user.id), parse_mode='HTML')
        else:
            await message.answer("<b>❌ Чек не был найден.</b>\n"
                                 "📃 Введите чек / номер покупки. Пример:\n"
                                 "▶ +123456789123\n"
                                 "▶ #123456789123", parse_mode='HTML')
            await StorageFunctions.here_search_receipt.set()
...
@dp.callback_query_handler(IsAdmin(), text_startswith="sber_pr", state="*")
async def change_user_sale(call: CallbackQuery, state: FSMContext):
    id = int(call.data.split(":")[1])
    ammount = int(call.data.split(":")[2])
    get_user = get_userx(user_id=id)
    update_userx(id, balance=get_user[4] + ammount)
    await call.message.delete()
    await bot.send_message(call.from_user.id, f"Пополнение одобрено")
    add_refillx(id, "Сбер", "1", f"tb: {id}",
                            str(ammount), "receipt", "Сбер", get_dates(),
                            int(time.time()))
    await bot.send_message(id, f"Ваш баланс пополнен на {ammount} руб.")

@dp.callback_query_handler(IsAdmin(), text_startswith="sber_ot", state="*")
async def change_user_sale(call: CallbackQuery, state: FSMContext):
    id = int(call.data.split(":")[1])
    ammount = int(call.data.split(":")[2])
    await call.message.delete()
    await bot.send_message(call.from_user.id, f"Пополнение отклонено")
    await bot.send_message(id, f""""
✨ Заявка на пополнение баланса отклонена
Попробуйте снова
🌴 Обратитесь в тех.поддержку если есть вопросы, проблемы""")